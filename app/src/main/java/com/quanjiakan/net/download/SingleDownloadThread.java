...
//        }
    ...
    public void download() {
        ...
        try {
            ...
            String tempInfo = UpdateUtil.getFileInfoValue(mContext, MessageDigestUtil.getSHA1String(url_string+sumSize+"_"+threadNumber));//TODO 在保存和获取断点数据时，必须根据子线程来，所以Key中必须带有子线程编号
...
//            httpURLConnection.setDoOutput(true);
            ...
            if (code == HttpURLConnection.HTTP_OK || code == HttpURLConnection.HTTP_PARTIAL) {
                ...
                while ((len = inputStream.read(cache)) != -1) {
                    ...
                    //TODO 记录下当前线程的断点
                    UpdateUtil.setFileInfoValue(mContext,MessageDigestUtil.getSHA1String(url_string+sumSize+"_"+threadNumber),
