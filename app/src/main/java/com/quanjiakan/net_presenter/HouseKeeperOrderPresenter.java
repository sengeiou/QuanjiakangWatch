...
import com.quanjiakan.net.retrofit.service.post.string.RxPostHouseKeeperOrderEncryptService;
import com.quanjiakan.net.retrofit.service.post.string.RxPostOrderVerifyStringService;
...
public class HouseKeeperOrderPresenter implements IBasePresenter {
    ...
    public void doGetHouseKeeperOrderEncrypt(final HouseKeeperOrderActivity activityMvp){
        HashMap<String, String> params = (HashMap<String, String>) activityMvp.getParamter(IPresenterBusinessCode.HOUSE_KEEPER_ORDER_ENCRYPT);
        if (params == null) {
            //TODO 控制无效的参数，需要针对不同的业务进行区分，当业务本身即为Get,不需要参数时，这个判断与回调也就不需要了
            activityMvp.onError(IPresenterBusinessCode.HOUSE_KEEPER_ORDER_ENCRYPT, 200, activityMvp.getString(R.string.error_common_net_invalid_parameters));
            return;
        }
        //
        activityMvp.showMyDialog(IPresenterBusinessCode.HOUSE_KEEPER_ORDER_ENCRYPT);

        Retrofit retrofit = Retrofit2Util.getRetrofitStringResult(IHttpUrlConstants.BASEURL_PAY_NO_PORT);
        RxPostHouseKeeperOrderEncryptService rxGetRequest = retrofit.create(RxPostHouseKeeperOrderEncryptService.class);
        rxGetRequest.getHouseKeeperOrderEncrypt(
                params.get(IParamsName.PARAMS_COMMON_MEMBERID),
                params.get(IParamsName.PARAMS_COMMON_PLATFORM),
                params.get(IParamsName.PARAMS_COMMON_TOKEN),

                params.get(IParamsName.PARAMS_HOUSE_KEEPER_ENCRYPT_NOTE),
                params.get(IParamsName.PARAMS_HOUSE_KEEPER_ENCRYPT_BEGINDATE),
                params.get(IParamsName.PARAMS_HOUSE_KEEPER_ENCRYPT_ENDDATE),
                params.get(IParamsName.PARAMS_HOUSE_KEEPER_ENCRYPT_COMPANYID),
                params.get(IParamsName.PARAMS_HOUSE_KEEPER_ENCRYPT_ADDRESS),
                params.get(IParamsName.PARAMS_HOUSE_KEEPER_ENCRYPT_ORDER_USERNAME),
                params.get(IParamsName.PARAMS_HOUSE_KEEPER_ENCRYPT_CIPHERTEXT)
        )
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<String>() {
                    @Override
                    public void onCompleted() {
                        activityMvp.dismissMyDialog(IPresenterBusinessCode.HOUSE_KEEPER_ORDER_ENCRYPT);
                    }

                    @Override
                    public void onError(Throwable e) {
                        activityMvp.dismissMyDialog(IPresenterBusinessCode.HOUSE_KEEPER_ORDER_ENCRYPT);
                        activityMvp.onError(IPresenterBusinessCode.HOUSE_KEEPER_ORDER_ENCRYPT, 200, e.getMessage());
                    }

                    @Override
                    public void onNext(String response) {
                        LogUtil.e("HOUSE_KEEPER_ORDER_ENCRYPT" + response);
                        activityMvp.dismissMyDialog(IPresenterBusinessCode.HOUSE_KEEPER_ORDER_ENCRYPT);
                        activityMvp.onSuccess(IPresenterBusinessCode.HOUSE_KEEPER_ORDER_ENCRYPT, 200, response);
                    }
                });
    }
    ...
    public void verifyAliPaymentEncryptResult(final HouseKeeperOrderActivity activityMvp){
        HashMap<String, String> params = (HashMap<String, String>) activityMvp.getParamter(IPresenterBusinessCode.ALI_PAY_VERIFY_RESULT_ENCRYPT);
        if (params == null) {
            //TODO 控制无效的参数，需要针对不同的业务进行区分，当业务本身即为Get,不需要参数时，这个判断与回调也就不需要了
            activityMvp.onError(IPresenterBusinessCode.ALI_PAY_VERIFY_RESULT_ENCRYPT, 200, activityMvp.getString(R.string.error_common_net_invalid_parameters));
            return;
        }
        //
        activityMvp.showMyDialog(IPresenterBusinessCode.ALI_PAY_VERIFY_RESULT_ENCRYPT);

        Retrofit retrofit = Retrofit2Util.getRetrofitStringResult(IHttpUrlConstants.BASEURL_PAY_NO_PORT);
        RxPostOrderVerifyStringService rxGetRequest = retrofit.create(RxPostOrderVerifyStringService.class);
        rxGetRequest.doGetHouseKeeperOrderString(params.get(IParamsName.PARAMS_COMMON_MEMBERID),
                params.get(IParamsName.PARAMS_HOUSE_KEEPER_ENCRYPT_ORDERID),
                params.get(IParamsName.PARAMS_COMMON_PLATFORM),
                params.get(IParamsName.PARAMS_COMMON_TOKEN)
        )
                .subscribeOn(Schedulers.io())
                ...
                .subscribe(new Subscriber<String>() {
                    @Override
                    public void onCompleted() {
                        activityMvp.dismissMyDialog(IPresenterBusinessCode.ALI_PAY_VERIFY_RESULT_ENCRYPT);
                    }

                    @Override
                    public void onError(Throwable e) {
                        activityMvp.dismissMyDialog(IPresenterBusinessCode.ALI_PAY_VERIFY_RESULT_ENCRYPT);
                        activityMvp.onError(IPresenterBusinessCode.ALI_PAY_VERIFY_RESULT_ENCRYPT, 200, e.getMessage());
                    }

                    @Override
                    public void onNext(String response) {
                        LogUtil.e("ALI_PAY_VERIFY_RESULT_ENCRYPT" + response);
                        activityMvp.dismissMyDialog(IPresenterBusinessCode.ALI_PAY_VERIFY_RESULT_ENCRYPT);
                        activityMvp.onSuccess(IPresenterBusinessCode.ALI_PAY_VERIFY_RESULT_ENCRYPT, 200, response);
                    }
                });
    }
    ...
    public void verifyWechatPaymentEncryptResult(final HouseKeeperOrderActivity activityMvp){
        HashMap<String, String> params = (HashMap<String, String>) activityMvp.getParamter(IPresenterBusinessCode.WECHAT_PAY_VERIFY_RESULT_ENCRYPT);
        if (params == null) {
            //TODO 控制无效的参数，需要针对不同的业务进行区分，当业务本身即为Get,不需要参数时，这个判断与回调也就不需要了
            activityMvp.onError(IPresenterBusinessCode.WECHAT_PAY_VERIFY_RESULT_ENCRYPT, 200, activityMvp.getString(R.string.error_common_net_invalid_parameters));
            return;
        }
        //
        activityMvp.showMyDialog(IPresenterBusinessCode.WECHAT_PAY_VERIFY_RESULT_ENCRYPT);
        ...
        rxGetRequest.doGetHouseKeeperOrderString(params.get(IParamsName.PARAMS_COMMON_MEMBERID),
                ...
                .subscribe(new Subscriber<String>() {
                    @Override
                    public void onCompleted() {
                        activityMvp.dismissMyDialog(IPresenterBusinessCode.WECHAT_PAY_VERIFY_RESULT_ENCRYPT);
                    }

                    @Override
                    public void onError(Throwable e) {
                        activityMvp.dismissMyDialog(IPresenterBusinessCode.WECHAT_PAY_VERIFY_RESULT_ENCRYPT);
                        activityMvp.onError(IPresenterBusinessCode.WECHAT_PAY_VERIFY_RESULT_ENCRYPT, 200, e.getMessage());
                    }

                    @Override
                    public void onNext(String response) {
                        LogUtil.e("WECHAT_PAY_VERIFY_RESULT_ENCRYPT" + response);
                        activityMvp.dismissMyDialog(IPresenterBusinessCode.WECHAT_PAY_VERIFY_RESULT_ENCRYPT);
                        activityMvp.onSuccess(IPresenterBusinessCode.WECHAT_PAY_VERIFY_RESULT_ENCRYPT, 200, response);
                    }
                });
    }
}
